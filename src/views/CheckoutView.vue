...
<template>
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-gray-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-850 transition-colors duration-300 py-12 px-6">
    <div class="max-w-4xl mx-auto">
      ...
      <h1 class="text-3xl md:text-5xl font-black text-[var(--text-color)] mb-8 md:mb-12 tracking-tighter">{{ language.translateDynamic('Secure Checkout') }}</h1>
      ...
      <div class="grid md:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          ...
          <div v-if="currentStep === 1" class="space-y-6 animate-fade-in">
            ...
            <div>
              <label class="block text-xs font-black uppercase text-[var(--text-muted)] mb-2 tracking-widest">{{ language.translateDynamic('Full Name') }}</label>
            ...
            <div>
              <label class="block text-xs font-black uppercase text-[var(--text-muted)] mb-2 tracking-widest">{{ language.translateDynamic('Shipping Address') }}</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-black uppercase text-[var(--text-muted)] mb-2 tracking-widest">{{ language.translateDynamic('City') }}</label>
              ...
              <div>
                <label class="block text-xs font-black uppercase text-[var(--text-muted)] mb-2 tracking-widest">{{ language.translateDynamic('Zip Code') }}</label>
          ...
          <div v-if="currentStep === 2" class="space-y-6 animate-fade-in">
            ...
            <div class="grid gap-4">
              <label class="flex items-center gap-4 p-5 border-2 rounded-2xl cursor-pointer transition-all duration-500 shadow-sm" 
                ...
                <div class="flex-1">
                  <div class="font-black text-[var(--text-color)] uppercase tracking-tight">{{ language.translateDynamic('Credit Card') }}</div>
                  <div class="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">{{ language.translateDynamic('Safe & Secure Payment') }}</div>
                </div>
              ...
              <label class="flex items-center gap-4 p-5 border-2 rounded-2xl cursor-pointer transition-all duration-500 shadow-sm" 
                ...
                <div class="flex-1">
                  <div class="font-black text-[var(--text-color)] uppercase tracking-tight">{{ language.translateDynamic('PayPal') }}</div>
                  <div class="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">{{ language.translateDynamic('One-click experience') }}</div>
                </div>
                <div class="text-3xl">🌍</div>
              </label>
              ...
              <label class="flex items-center gap-4 p-5 border-2 rounded-2xl cursor-pointer transition-all duration-500 shadow-sm" 
                ...
                <div class="flex-1">
                  <div class="font-black text-[var(--text-color)] uppercase tracking-tight">{{ language.translateDynamic('Cash on Delivery') }}</div>
                  <div class="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">{{ language.translateDynamic('Pay at your doorstep') }}</div>
                </div>
                <div class="text-3xl">💵</div>
              </label>
            </div>
        ...
        <div class="bg-[var(--card-bg)] rounded-2xl shadow-2xl p-8 border border-[var(--border-color)] relative h-fit">
          ...
          <ul class="space-y-6">
            <li v-for="item in selectedItems" :key="item.id" class="flex justify-between items-center gap-4">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <span class="text-xs font-bold text-[var(--text-color)] truncate uppercase tracking-tighter">{{ language.translateDynamic(item.title) }}</span>
              </div>
          ...
          <div class="mt-8 border-t border-[var(--border-color)] pt-6 space-y-4">
            ...
            <div class="flex justify-between font-black text-2xl tracking-tighter text-[var(--promo-color)]">
              <span class="uppercase text-xs self-center text-[var(--text-color)] tracking-widest">{{ language.translateDynamic('Total Pay') }}</span>
