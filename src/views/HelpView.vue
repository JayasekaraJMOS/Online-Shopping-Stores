  ...
  <main class="min-h-screen bg-(--bg-color) text-(--text-color)">
    ...
    <section class="relative overflow-hidden" style="background: linear-gradient(135deg, #0F172A, #1E293B, #1E3A8A);">
      ...
      <div class="max-w-7xl mx-auto px-4 py-20 text-center relative z-10 text-white">
        ...
        <div class="max-w-xl mx-auto flex bg-white/10 border border-white/20 rounded-xl overflow-hidden backdrop-blur-sm">
          <input type="text" :placeholder="language.translateDynamic('Search help articles...')" class="grow bg-transparent text-white placeholder:text-white/50 px-5 py-3 focus:outline-none text-sm font-medium" />
          <button class="bg-(--accent-color) px-6 py-3 font-black text-sm text-white hover:opacity-90 transition-opacity">{{ language.translateDynamic('Search') }}</button>
        </div>
      </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-4 py-12">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div
          ...
          class="bg-(--card-bg) border border-(--border-color) rounded-2xl p-6 flex flex-col gap-3 hover:border-(--accent-color) hover:shadow-xl transition-all duration-300 group text-center items-center"
        >
          ...
          <div>
            ...
            <p class="text-xs text-(--text-muted) leading-relaxed">{{ language.translateDynamic(c.desc) }}</p>
          </div>
          <button class="mt-auto w-full bg-(--accent-color) text-white font-black text-xs uppercase tracking-widest py-2.5 rounded-xl hover:opacity-90 transition-opacity">
      ...
      <div class="max-w-3xl mx-auto px-4 py-14">
        ...
        <div class="space-y-3">
          <div
            ...
            class="border border-(--border-color) rounded-2xl overflow-hidden transition-all duration-300"
            :class="openFaq === i ? 'border-(--accent-color) shadow-md' : 'hover:border-(--accent-color)/50'"
          >
            <button
              @click="toggleFaq(i)"
            ...
            >
              ...
              <span class="text-(--accent-color) font-black text-lg shrink-0 transition-transform duration-300"
                :class="openFaq === i ? 'rotate-45' : ''">+</span>
            </button>
            <div v-if="openFaq === i" class="px-5 pb-5">
              <p class="text-sm text-(--text-muted) leading-relaxed border-t border-(--border-color) pt-4">{{ language.translateDynamic(faq.a) }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="max-w-3xl mx-auto px-4 py-14">
      ...
      <div v-if="submitted" class="bg-[#ECFDF5] border-2 border-[#059669] rounded-2xl p-10 text-center">
        ...
        <p class="text-sm text-(--text-muted)">{{ language.translateDynamic('Our support team will get back to you within 24 hours.') }}</p>
      ...
      <form v-else @submit.prevent="submitForm" class="bg-(--card-bg) border border-(--border-color) rounded-2xl p-8 space-y-5">
        ...
        <button type="submit"
          class="w-full bg-(--accent-color) text-white font-black text-xs uppercase tracking-widest py-4 rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2"
          :disabled="submitting"
        >
